		...
		this.hooks.startup.tap(
			...
			(source, { chunk, hash, chunkGraph }) => {
				...
				if (runtimeRequirements.has(RuntimeGlobals.startup)) {
					startup = Template.asString([
						"// run modules when ready",
						`return ${RuntimeGlobals.startup}();`
					]);
				} else if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
					/** @type {string[]} */
					const buf = [];
					const runtimeRequirements = chunkGraph.getTreeRuntimeRequirements(
						chunk
					...
					startup = Template.asString(buf);
				}
				if (startup) {
					if (runtimeRequirements.has(RuntimeGlobals.delayStartup)) {
						return Template.asString([
							"// run startup preparation",
							`return ${RuntimeGlobals.delayStartup}().then(function() {`,
							Template.indent(startup),
							"})"
						]);
					}
					return startup;
